...
class MainView extends WatchUi.View {
    ...
    // decorative mini-tower of blocks, single START row.
    hidden function _drawMenu(dc) {
        ...
        // Decorative mini-tower
        var towerX = cx - 14;
        var palette = [0xFF3344, 0xFFCC22, 0x44FF55, 0x44CCFF, 0x8866FF, 0xFF44AA];
        for (var i = 0; i < 6; i++) {
            var off = (i % 2 == 0) ? -3 : 3;
            _drawBlock(dc, towerX + off, _sh * 58 / 100 - i * 7,
                       30, 6, palette[i], false);
        }

        if (_ctrl.hi > 0) {
            dc.setColor(0xFFCC22, Graphics.COLOR_TRANSPARENT);
            dc.drawText(cx, _sh * 64 / 100, Graphics.FONT_XTINY,
                        "BEST " + _ctrl.hi.format("%d"),
                        Graphics.TEXT_JUSTIFY_CENTER);
        }

        // Chess-style START row
        var rowH = (_sh * 13) / 100; if (rowH < 26) { rowH = 26; }
