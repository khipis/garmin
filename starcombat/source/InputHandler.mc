...
//
//   PLAY:
//     tilt (live)          → look around (in MainView.onTick)
//     UP                   → recalibrate (level the wrist neutral)
//     DOWN / SELECT / tap  → FIRE
//     ESC / BACK           → menu
//
//   OVER:
//     any key / tap        → restart / menu
//
// Touch: ignore native onSwipe; resolve every touch in
...
class InputHandler extends WatchUi.BehaviorDelegate {
    ...
    hidden function _pageFromTouch() {
        ...
        return (dt >= 0 && dt < SC_PAGE_GUARD_MS);
    }
    ...
    function onPreviousPage() {
        if (_v.ctrl.state == SC_PLAY && _pageFromTouch()) { return true; }
