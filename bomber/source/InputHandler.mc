// ═══════════════════════════════════════════════════════════════
// InputHandler.mc — Bomber input dispatch.
//
//   MENU:
//     UP / onPreviousPage    → prev row
//     DOWN / onNextPage      → next row
//     SELECT / onEnter / tap → activate row
//     ESC                    → exit app
//
//   PLAY:
//     UP key                 → move ↑
//     DOWN key               → move ↓
//     SELECT short           → place bomb at player
//     swipe ↑↓←→             → move that direction (only 4-way)
//     tap on player tile     → place bomb
//     tap on adjacent tile   → step toward that tile (single step)
//     hold ≥ 600 ms          → restart current run
//     ESC                    → menu
//
//   WIN/OVER:
//     SEL / tap              → next level / retry
//     ESC                    → menu
//
// As with our other recent games, native onSwipe is ignored and
// onDrag distinguishes tap (<30 px) from swipe with dominant-axis.
// ═══════════════════════════════════════════════════════════════
...
class InputHandler extends WatchUi.BehaviorDelegate {
    ...
    function onDrag(evt) {
        ...
        if (t == WatchUi.DRAG_TYPE_STOP && _dragActive) {
            ...
            if (adx < 30 && ady < 30) {
                if (dur >= 600) { _v.handleHold(xy[0], xy[1]); }
