...
//
// PLAY:
//   UP key                 → cursor PREV (scan order, white cells)
//   DOWN key               → cursor NEXT (scan order, white cells)
//   SELECT short           → cycle cell (NONE → BULB → X → NONE)
//   SELECT long / HOLD     → set/clear X mark on cursor
//   swipe ↑↓←→             → cursor 4-way (skips through walls)
//   tap on cell            → cycle that cell
//   hold on cell           → mark X on that cell
//   ESC                    → menu
//
// WIN:
...
//   ESC                    → menu
//
// Native onSwipe is ignored; we resolve every touch in onDrag with
// a 30 px tap window — sustained drag ≥ 500 ms without movement is
// "hold".  Same model as LightsOut / Nonogram.
// ═══════════════════════════════════════════════════════════════
...
class InputHandler extends WatchUi.BehaviorDelegate {
    ...
    function onDrag(evt) {
        ...
        if (t == WatchUi.DRAG_TYPE_STOP && _dragActive) {
            ...
            if (adx < 30 && ady < 30) {
                if (dur >= 500) { _v.handleHold(xy[0], xy[1]); }
